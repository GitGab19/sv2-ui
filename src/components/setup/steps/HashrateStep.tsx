...
export function HashrateStep({ data, updateData, onNext }: StepProps) {
  ...
  const [customUnit, setCustomUnit] = useState<'gh' | 'th' | 'ph'>('th');

  const getHashrateValue = (): number => {
    if (selectedPreset === 'custom') {
      const multiplier = customUnit === 'ph' 
        ? 1_000_000_000_000_000 
        : customUnit === 'th' 
          ? 1_000_000_000_000 
          : 1_000_000_000;
      return customHashrate * multiplier;
    }
  ...
  }, [hashrate, data.translator?.user_identity, data.translator?.enable_vardiff, data.translator?.aggregate_channels, updateData]);
      ...
      {selectedPreset === 'custom' && (
        <div className="p-4 rounded-xl border border-border bg-muted/50">
          ...
          <div className="flex gap-3">
            ...
            <select
              value={customUnit}
              onChange={(e) => setCustomUnit(e.target.value as 'gh' | 'th' | 'ph')}
